...
import { SMPPDeliveryReceipt, InvalidMessageError } from "./smpp_delivery_receipt.js";
  ...
  #handleMessageDeliveryReceipt() {
    this.#session.on("deliver_sm", (pdu) => {
      if (pdu.esm_class === smpp.ESM_CLASS.MC_DELIVERY_RECEIPT) {
        try {
          const deliveryReceipt = SMPPDeliveryReceipt.parse(pdu.short_message.message);
          this.#onSentCallback && this.#onSentCallback(deliveryReceipt);
        } catch (error) {
          if (error instanceof InvalidMessageError) {
            console.log("Unsupported delivery receipt message: ", pdu.short_message.message);
          }
        }
      } else {
